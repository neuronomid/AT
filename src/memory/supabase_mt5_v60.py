...
class SupabaseMT5V60Store:
    def __init__(self, db_url: str) -> None:
        self._db_url = db_url
        self._last_prune_at: datetime | None = None

    def insert_mt5_v60_bridge_snapshot(self, *, agent_name: str, snapshot: MT5V60BridgeSnapshot) -> str:
        with psycopg.connect(self._db_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    insert into public.mt5_v60_bridge_snapshots (
    ...
    def insert_mt5_v60_close_events(self, *, agent_name: str, close_events: Sequence[MT5V60CloseEvent]) -> None:
        if not close_events:
            return
        with psycopg.connect(self._db_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                for event in close_events:
                    cur.execute(
                        """
                        insert into public.mt5_v60_close_events (
                          event_id, agent_name, symbol, ticket_id, basket_id, closed_at, close_reason, event_payload
                        )
                        values (%s, %s, %s, %s, %s, %s, %s, %s)
    ...
    def insert_mt5_v60_runtime_decision(
        self,
        *,
        agent_name: str,
        decision_kind: str,
        ...
        with psycopg.connect(self._db_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    insert into public.mt5_v60_runtime_decisions (
    ...
    def insert_mt5_v60_bridge_command(
        self,
        *,
        agent_name: str,
        ...
        with psycopg.connect(self._db_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    insert into public.mt5_v60_bridge_commands (
    ...
    def insert_mt5_v60_bridge_ack(self, *, agent_name: str, ack: MT5V60ExecutionAck) -> str:
        with psycopg.connect(self._db_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    insert into public.mt5_v60_bridge_acks (
                ...
                cur.execute(
                    """
                    update public.mt5_v60_bridge_commands
                    set status = %s,
    ...
    def insert_mt5_v60_trade_reflection(
        self,
        *,
        agent_name: str,
        ...
        with psycopg.connect(self._db_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    insert into public.mt5_v60_trade_reflections (
    ...
    def upsert_mt5_v60_ticket_state(self, ticket: MT5V60TicketRecord) -> str:
        payload = ticket.model_dump(mode="json")
        with psycopg.connect(self._db_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    insert into public.mt5_v60_ticket_state (
                      ticket_id, symbol, side, basket_id, magic_number, entry_command_id,
                      is_open, opened_at, last_seen_at, current_price,
                    ...
                    values (%(ticket_id)s, %(symbol)s, %(side)s, %(basket_id)s, %(magic_number)s, %(entry_command_id)s,
                            %(is_open)s, %(opened_at)s, %(last_seen_at)s, %(current_price)s,
    ...
    def list_open_ticket_states(self, symbol: str | None = None) -> list[MT5V60TicketRecord]:
        sql = """
            select ticket_payload
            from public.mt5_v60_ticket_state
            where is_open = true
        """
        params: list[Any] = []
        ...
        with psycopg.connect(self._db_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                ...
                rows = cur.fetchall()
        return [MT5V60TicketRecord.model_validate(row["ticket_payload"]) for row in rows]

    def find_entry_command_payload(self, *, symbol: str, basket_id: str | None, magic_number: int | None) -> dict[str, Any] | None:
        if basket_id is None and magic_number is None:
            return None
        sql = """
            select command_payload
            from public.mt5_v60_bridge_commands
            where symbol = %s
        ...
        with psycopg.connect(self._db_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    select recorded_at
                    from public.mt5_v60_runtime_decisions
                    where symbol = %s
        ...
        with psycopg.connect(self._db_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    from public.mt5_v60_trade_reflections
                    where symbol = %s
    ...
    def _maybe_prune_snapshots(self, cur) -> None:
        ...
        if self._last_prune_at is not None and now < self._last_prune_at + timedelta(minutes=1):
            return
        cur.execute("select public.prune_mt5_v60_bridge_snapshots(interval '2 hours')")
