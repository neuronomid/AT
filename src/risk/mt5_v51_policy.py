...
class MT5V51RiskArbiter:
    ...
    def evaluate_entry(
        ...
        *,
        ...
        if decision.confidence < self._min_confidence:
            return MT5V51RiskDecision(approved=False, reason="Decision confidence is below minimum.", risk_posture=risk_posture)
        if self.snapshot_is_stale(snapshot):
            return MT5V51RiskDecision(approved=False, reason="Snapshot is stale.", risk_posture=risk_posture)
        if snapshot.spread_bps is not None and snapshot.spread_bps > self._max_spread_bps:
            return MT5V51RiskDecision(approved=False, reason="Spread is wider than the configured limit.", risk_posture=risk_posture)
        if pending_symbol_command:
            return MT5V51RiskDecision(approved=False, reason="A pending command already exists for the symbol.", risk_posture=risk_posture)
        ...
        if registry.has_open_position(snapshot.symbol):
            return MT5V51RiskDecision(approved=False, reason="A BTCUSD ticket is already open.", risk_posture=risk_posture)
    ...
    def snapshot_is_stale(self, snapshot: MT5V51BridgeSnapshot) -> bool:
    ...
    def _daily_loss_triggered(self, snapshot: MT5V51BridgeSnapshot) -> bool:
