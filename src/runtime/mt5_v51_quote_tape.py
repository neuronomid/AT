...
from data.mt5_v51_schemas import MT5V51BridgeSnapshot
...
class MT5V51QuoteTape:
    def __init__(self, *, max_samples: int = 512) -> None:
        self._samples: deque[MT5V51QuoteSample] = deque(maxlen=max_samples)
        self._last_key: tuple[datetime, datetime, Decimal, Decimal, float | None] | None = None

    def ingest(self, snapshot: MT5V51BridgeSnapshot) -> None:
    ...
    def build_payload(
        ...
        *,
        ...
        recent_1m = self._window(current_received_at, seconds=60)
        recent_10s = self._window(current_received_at, seconds=10)
        current_spread = snapshot.spread_bps
        spreads = [sample.spread_bps for sample in recent_1m if sample.spread_bps is not None]
        ...
        return {
            "spread_percentile_1m": self._spread_percentile(current_spread=current_spread, spreads=spreads),
            "spread_to_1m_atr_ratio": self._spread_to_atr_ratio(current_spread=current_spread, one_minute_atr_bps=one_minute_atr_bps),
            ...
            "mid_drift_bps_10s": (
                self._drift_bps(current=float(snapshot.midpoint), reference=float(oldest.midpoint)) if oldest is not None else 0.0
            ),
            "sample_count_1m": len(recent_1m),
    ...
    def _trim(self, anchor: datetime) -> None:
        cutoff = anchor - timedelta(minutes=3)
        while self._samples and self._samples[0].received_at < cutoff:
            self._samples.popleft()

    def _window(self, anchor: datetime, *, seconds: int) -> list[MT5V51QuoteSample]:
    ...
    def _spread_to_atr_ratio(self, *, current_spread: float | None, one_minute_atr_bps: float | None) -> float | None:
        if current_spread is None or one_minute_atr_bps is None or one_minute_atr_bps <= 0:
            return None
        return round(current_spread / one_minute_atr_bps, 4)
