    ...
    *,
    ...
    if decision.action == "enter_long":
        ...
        if (
            ...
            and not _coerce_bool(twenty.get("long_trigger_ready"))
            and not _coerce_bool(twenty.get("long_continuation_ready"))
        ):
            return "20s tape is no longer aligned with the long entry."
        if (
    ...
    if (
        ...
        and not _coerce_bool(twenty.get("short_trigger_ready"))
        and not _coerce_bool(twenty.get("short_continuation_ready"))
    ):
        return "20s tape is no longer aligned with the short entry."
    if (
        _coerce_float(one.get("ema_gap_bps")) >= 0
